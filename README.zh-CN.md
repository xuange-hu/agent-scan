...
面向 **MCP Server · Agent Skills · AGENTS.md / 规则文件 · Agent 工具代码** 的静态安全扫描器。
...
```bash
npx github:agent-scan/agent-scan path/to/repo   # 无需安装
npm install -g github:agent-scan/agent-scan     # 或全局安装
```
...
- **纯静态**：不执行被扫代码、不联网、不依赖大模型，规则全部可审阅。
...
- 只扫**文件**。远程 MCP Server 运行时的工具描述投毒不在 v0.1 范围（见 Roadmap 的动态探测模式）。
